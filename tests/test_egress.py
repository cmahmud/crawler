...
async def test_allows_public_destination() -> None:
    policy = EgressPolicy()
    assert await policy.validate("https://Example.com/a#x", resolver=static_resolver) == "https://example.com/a"


@pytest.mark.asyncio
@pytest.mark.parametrize("url", ["http://127.0.0.1/", "http://169.254.169.254/latest/meta-data/", "http://10.0.0.1/", "http://[::1]/", "http://localhost/"])
